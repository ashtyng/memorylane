...
app.get('/api/users/:id/memories', (req, res) => {
  let id = parseInt(req.params.id);
  knex('users').join('memories','users.id','memories.user_id')
    .where('users.id',id)
    .orderBy('created','desc')
    .select('memory','username','name','created').then(memories => {
...
app.post('/api/users/:id/memories', (req, res) => {
  let id = parseInt(req.params.id);
  knex('users').where('id',id).first().then(user => {
    return knex('memories').insert({user_id: id, memory:req.body.memory, created: new Date()});
  }).then(ids => {
    return knex('memories').where('id',ids[0]).first();
  }).then(memory => {
...
app.get('/api/memories/search', (req, res) => {
  ...
  knex('users').join('memories','users.id','memories.user_id')
    .whereRaw("MATCH (memory) AGAINST('" + req.body.keywords + "')")
    .orderBy('created','desc')
    .limit(limit)
    .offset(offset)
    .select('memory','username','name','created').then(memories => {
